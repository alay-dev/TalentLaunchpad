            ...
            <div className='w-full'>
                ...
                <div className="bg-white rounded-lg mt-8 shadow-[0_6px_15px_rgba(64,79,104,.05)]   p-5">
                    {appliedJobs?.data?.length === 0 &&
                        <>
                            <Image className='m-auto' alt="no data" width={400} height={400} src={"/assets/noData.svg"} />
                            <p className='text-center text-lg text-gray-500' >You have not applied to any jobs yet!</p>
                        </>
                    }
                    {appliedJobs?.data?.length !== 0 &&
                        <table className='w-full'>
                            ...
                            <tbody className='mt-3'>
                                {appliedJobs.data.map(item => {
                                    return <tr key={item.job_title}>
                                        <td className='py-4 px-2 flex gap-4 items-start' >
                                            <Image alt="company logo" width={50} height={50} src={`${UNIVERSAL.BASEURL}/company/${item?.company_logo}`} />
                                            <div>
